...
prompt_mode_b = PromptTemplate(
    ...
    template=
"""
Responde la pregunta de forma precisa y cuidadosa basándote ÚNICAMENTE en el CONTEXTO proporcionado.
No uses conocimientos externos. Si la información no está en el CONTEXTO, indícalo explícitamente.

Devuelve SIEMPRE un JSON válido con exactamente estos dos campos:

- "answer": respuesta directa a la pregunta. Texto plano (string), sin subsecciones ni formato anidado.
- "sources": lista de las referencias del CONTEXTO utilizadas. Cada referencia debe incluir:
  {{
    "source": "nombre del documento",
    "page": número_de_página,
    "reason": "cómo esta fuente respalda tu respuesta"
  }}
  Si no hay información suficiente → "sources": [] y explícalo en "answer".
